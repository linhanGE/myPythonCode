...
#    tend = -1
    ...
    end_index = -1
    mfpcnr = mfpdata[start_index:end_index,cnr]
    mfpx = mfpdata[start_index:end_index,x]
    mfpy = mfpdata[start_index:end_index,y]
    mfpz = mfpdata[start_index:end_index,z]
    ...
    def calPath(begin,end,x,y,z):
        ...
        ssum = np.sum(np.sqrt(np.einsum('ij,ij->j',s,s)))   # np.einsum is used to sum squre of each element in a column
        return ssum
		
    def calPathx(begin,end,x):
        diffx = np.diff(x[begin:end])
        s = np.abs(diffx) 
        sx = np.sum(s)   
        return sx
    
    def calPathy(begin,end,y):
        diffy = np.diff(y[begin:end])
        s = np.abs(diffy)
        sy = np.sum(s)   
        return sy
    
    def calPathz(begin,end,z):
        diffz = np.diff(z[begin:end])
        s = np.abs(diffz) 
        sz = np.sum(s)   
        return sz
    ...
    mfp = np.zeros(lencon)
    mfp_x = np.zeros(lencon)
    mfp_y = np.zeros(lencon)
    mfp_z = np.zeros(lencon)
    
    for i in np.arange(lencon):
        begin = next(itconBegin)
        end = next(itconEnd)
        mfp[i] = calPath(begin,end,mfpx,mfpy,mfpz)
        mfp_x[i] = calPathx(begin,end,mfpx)
        mfp_y[i] = calPathy(begin,end,mfpy) 
        mfp_z[i] = calPathz(begin,end,mfpz)
        meanmfp = np.average(mfp)
        meanmfpx = np.average(mfp_x)
        meanmfpy = np.average(mfp_y)
        meanmfpz = np.average(mfp_z)
    return meanmfp,meanmfpx, meanmfpy,meanmfpz
