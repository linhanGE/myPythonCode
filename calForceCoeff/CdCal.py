...
with open(forces_file,"r") as datafile:
    for line in datafile:
        ...
        time += [data_dict['time']]
        drag += [data_dict['force']['pressure'][2] + data_dict['force']['viscous'][2]]
        lifty += [data_dict['force']['pressure'][1] + data_dict['force']['viscous'][1]]
        liftx += [data_dict['force']['pressure'][0] + data_dict['force']['viscous'][0]]
        moment += [data_dict['moment']['pressure'][2] + data_dict['moment']['viscous'][2]]
datafile.close()

outputfile = open('forces.txt','w')
for i in range(0,len(time)):
#    outputfile.write(str(time[i])+' '+str(lifty[i])+' '+str(lifty[i])+' '+str(drag[i])+' '+str(moment[i])+'\n')
    outputfile.write(str(time[i])+str(drag[i])+'\n')
outputfile.close()
